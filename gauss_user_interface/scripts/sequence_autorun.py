...
from gauss_msgs.msg import SequenceAutorunStatus
from gauss_msgs.srv import SetInt
from gauss_msgs.srv import SetSequenceAutorun
...
from gauss_user_interface.sequences.sequence import Sequence
...
class SequenceAutorun:
    ...
    def send_calibration_command(self):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun send_calibration_command")
        try:
            rospy.wait_for_service('/gauss/calibrate_motors', 0.1)
            start_calibration = rospy.ServiceProxy('/gauss/calibrate_motors', SetInt)
    ...
    def activate_learning_mode(self, activate):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun activate_learning_mode")
        try: 
            rospy.wait_for_service('/gauss/activate_learning_mode', 0.1)
    ...
    def execute_sequence(self, seq_id):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun execute_sequence")
        goal = SequenceGoal()
    ...
    def send_robot_to_sleep_position(self):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun send_robot_to_sleep_position")
    ...
    def execute_sequence_autorun_thread(self):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun execute_sequence_autorun_thread")
        self.sequence_execution_index = -1
        while True:
            rospy.sleep(0.1)
            if self.activated:
    ...
    def __init__(self, logger):
        ...
        rospy.loginfo('sequence_autorun_status_file is: '+ self.sequence_autorun_status_file)
        ...
        self.hardware_status_subscriber = rospy.Subscriber(
                '/gauss/hardware_status', HardwareStatus, self.sub_hardware_status)

        self.learning_mode_on = None
        self.learning_mode_subscriber = rospy.Subscriber(
                '/gauss/learning_mode', Bool, self.sub_learning_mode)

        # Wait for sequence action server to finish setup
        self.sequence_action_client = actionlib.SimpleActionClient('gauss/sequences/execute', SequenceAction)
        self.sequence_action_client.wait_for_server()
        
        self.sequence_autorun_status_publisher = rospy.Publisher(
                '/gauss/sequences/sequence_autorun_status', SequenceAutorunStatus, queue_size=10)
        ...
        self.set_sequence_autorun_server = rospy.Service(
                '/gauss/sequences/set_sequence_autorun', SetSequenceAutorun, self.callback_set_sequence_autorun)

        self.trigger_sequence_autorun = rospy.Service(
                '/gauss/sequences/trigger_sequence_autorun', SetInt, self.callback_trigger_sequence_autorun)
    ...
    def callback_set_sequence_autorun(self, req):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun callback_set_sequence_autorun")
        if self.activated:
    ...
    def callback_trigger_sequence_autorun(self, req):
        self.gauss_ros_logger.publish_log_status("INFO", "SequenceAutorun callback_trigger_sequence_autorun")
        if not self.enabled:
