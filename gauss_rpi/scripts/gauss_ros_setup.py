...
from gauss_msgs.msg import ProcessState
from gauss_msgs.srv import ManageProcess
...
class Process:
    ...
    def __init__(self, name, cmd, args=[], launch_on_startup=False, 
            delay_before_start=0.0, dependencies=[]):
        self.name = name
        self.cmd = cmd
        self.args = args
...
class GaussRosSetup:
    ...
    def __init__(self):
        ...
        self.process_state_publisher = rospy.Publisher(
                '/gauss/rpi/process_state', ProcessState, queue_size=1)
        ...
        self.manage_process_server = rospy.Service(
                '/gauss/rpi/manage_process', ManageProcess, self.callback_manage_process)
    ...
    def create_processes(self):
        ...
        for p in self.process_config:
            self.process_list.append(Process(name=p['name'], cmd=p['cmd'],args=p['args'],
                launch_on_startup=p['launch_on_startup'], delay_before_start=p['delay_before_start'],
...
if __name__ == '__main__':
    pass
    #rospy.init_node('gauss_ros_setup')
    #n = GaussRosSetup()
    #rospy.spin()
