...
#include "gauss_driver/dynamixel/dxl_driver.h"
#include <vector>
...
class XL320Driver : public DxlDriver {
    ...
    public:
        ...
        int syncWritePgain          (std::vector<uint8_t> &id_list, std::vector<uint32_t> &led_list);
        ...
        int syncReadPgain  (std::vector<uint8_t> &id_list, std::vector<uint32_t> &hw_error_list);
};
