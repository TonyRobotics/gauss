...
int DxlCommunication::init(int hardware_version)
{
    ...
    ros::param::get("/gauss/motors/dxl_required_motors", required_dxl_ids);
    ros::param::get("/gauss/motors/dxl_authorized_motors", allowed_dxl_ids);
        ...
        for (int i = 0 ; i < required_dxl_ids.size() ; i++) {
            if      (required_dxl_ids.at(i) == m5_1.getId()) { m5_1.enable(); }
                ...
                debug_error_message = "Incorrect configuration : Wrong ID (" + std::to_string(required_dxl_ids.at(i)) 
                    + ") given in Ros Param /gauss_motors/dxl_required_motors. You need to fix this !";
                ROS_ERROR("%s", debug_error_message.c_str());
                return -1;
            }
        }
    ...
    if (required_dxl_ids.size() == 0) {
        debug_error_message = "Incorrect configuration : Ros Param /gauss_motors/dxl_required_motors "
...
void DxlCommunication::startHardwareControlLoop(bool limited_mode)
{
    ...
    if (p_gain_custom) {
        ...
        ROS_INFO("xl320_motor_list size: %d", xl320_motor_list.size());
        //write
        std::vector<uint32_t> xl320_pgain_write_list;
        for (int i = 0; i < xl320_motor_list.size(); i++) {
            xl320_pgain_write_list.push_back(50);
        }   

        int xl320_result = xl320->syncWritePgain(xl320_id_list, xl320_pgain_write_list);

        if (xl320_result != COMM_SUCCESS ) {
            ROS_WARN("Failed to write pgain");
        }

        ros::Duration(0.01).sleep();
        //read
        std::vector<uint32_t> pgain_read_list;
        int read_pgain_result = xl320->syncReadPgain(xl320_id_list, pgain_read_list);
        if (read_pgain_result == COMM_SUCCESS) {
            for (int i = 0; i < xl320_motor_list.size(); i++) {
                ROS_INFO("get pgain %d", pgain_read_list.at(i));
            }
        }
    }
    ...
    if (ros::Time::now().toSec() - time_hw_data_last_write > 1.0/hw_data_write_frequency) {
        ...
        if (torque_on) {
            ...
            if (write_tool_enable && is_tool_connected) {
                // ROS_INFO("now do gripper command");
                std_msgs::String log_msg;
                if(is_gripper_opening){
                    log_msg.data = std::string("hardwareControlWrite INFO before gripper open command");
                    g_roslogger_pub.publish(log_msg);
                }else if(is_gripper_closing){
                    log_msg.data = std::string("hardwareControlWrite INFO before gripper close command");
                    g_roslogger_pub.publish(log_msg);
                }
                ...
                if(is_gripper_opening){
                    log_msg.data = std::string("hardwareControlWrite INFO after gripper open command");
                    g_roslogger_pub.publish(log_msg);
                }else if(is_gripper_closing){
                    log_msg.data = std::string("hardwareControlWrite INFO after gripper close command");
                    g_roslogger_pub.publish(log_msg);
                }
                ...
                if (write_tool_velocity_result != COMM_SUCCESS || 
                    ...
                    log_msg.data = std::string("Failed to write on tool");
                    g_roslogger_pub.publish(log_msg);
                }
                else {
                    //write success
                    // ROS_INFO("do gripper command success");
                    if(is_gripper_opening){
                        log_msg.data = std::string("hardwareControlWrite INFO do gripper open command success");
                        g_roslogger_pub.publish(log_msg);
                    }else if(is_gripper_closing){
                        log_msg.data = std::string("hardwareControlWrite INFO do gripper close command success");
                        g_roslogger_pub.publish(log_msg);
                    }
                    ...
                    // if(is_gripper_closing) is_gripper_closing = false;                                  
                }
                //stop when grasp something
                if(stop_gripper_enable) {
                    std::stringstream ss;
                    ss <<"hardwareControlWrite INFO got stop_gripper_enable, position: ";
                    int32_t position = tool.getPositionState();
                    ss<< position;
                    log_msg.data = ss.str() ;
                    // int32_t torque = tool.getTorqueState();
                    // ROS_INFO("get position: %d", position);
                    // ROS_INFO("get torque: %d", torque);

                    ros::Duration(0.005).sleep();
                    int write_tool_position_result = xl320->setGoalPosition(tool.getId(), position);
                    ros::Duration(0.005).sleep();
                    if(write_tool_position_result != COMM_SUCCESS) {
                        ROS_ERROR("Failed to write on tool, setGoalPosition");
                    }
...
int DxlCommunication::pingAndSetTool(uint8_t id, std::string name)
{
    ...
    // temp annotation until solve dxl ping communication conflict problem !!!
    /*
    while (hw_is_busy) { 
        ros::Duration(TIME_TO_WAIT_IF_BUSY).sleep();    
    }
...
int DxlCommunication::openGripper(uint8_t id, uint16_t open_position, uint16_t open_speed, uint16_t open_hold_torque)
{
    ...
    std_msgs::String log_msg;
    std::stringstream ss;
    ss<<"DxlCommunication INFO openGripper current position: ";
    ss<<tool.getPositionState();
    log_msg.data = ss.str();
    g_roslogger_pub.publish(log_msg);

    if(tool.getPositionState() >=591)
    {
    ...
    while(true) {
        ...
        if(is_gripper_opening || is_gripper_closing ) //如果当前有命令，则等待
        {
            ros::Duration(0.1).sleep();
        }else{
            gripper_command_mutex.lock();
            break;
        }
    }
    // ROS_INFO("DxlCommunication openGripper");
    log_msg.data = std::string("DxlCommunication INFO openGripper");
    g_roslogger_pub.publish(log_msg);
    ...
    ros::Duration(seconds_to_wait + 0.30).sleep();
    log_msg.data = std::string("DxlCommunication INFO openGripper sleep over");
    g_roslogger_pub.publish(log_msg);
    ...
    while(true) {
        ...
        if(is_gripper_opening) //如果当前未执行完，则sleep
        {
            ros::Duration(0.1).sleep();
        }else{
            break;
    ...
    // ROS_INFO("DxlCommunication INFO openGripper now return");
    log_msg.data = std::string("DxlCommunication INFO openGripper now return");
    g_roslogger_pub.publish(log_msg);

    gripper_command_mutex.unlock();
    return GRIPPER_STATE_OPEN;
}
...
int DxlCommunication::closeGripper(uint8_t id, uint16_t close_position, uint16_t close_speed, uint16_t close_hold_torque, uint16_t close_max_torque)
{
    ...
    std_msgs::String log_msg;
    std::stringstream ss;
    ss<<"DxlCommunication INFO closeGripper current position: ";
    ss<<tool.getPositionState();
    log_msg.data = ss.str();
    g_roslogger_pub.publish(log_msg);

    if(tool.getPositionState() <= 361)
    {
    ...
    int position_command = (close_position < 350) ? 350 : close_position;
    // ROS_INFO("DxlCommunication closeGripper");
    log_msg.data = std::string("DxlCommunication INFO closeGripper");
    g_roslogger_pub.publish(log_msg);
    // set gripper pos, vel and torque
    tool.setVelocityCommand(close_speed);
    tool.setPositionCommand(position_command);
    tool.setTorqueCommand(close_hold_torque);
    is_gripper_closing = true;
    write_tool_enable = true;

    // calculate close duration
    int dxl_speed = close_speed * XL320_STEPS_FOR_1_SPEED; // position . sec-1
    int dxl_steps_to_do = abs(position_command - tool.getPositionState()); // position
    double seconds_to_wait = (double) dxl_steps_to_do / (double) dxl_speed; // sec

    ros::Duration(seconds_to_wait + 0.50).sleep();
    stop_gripper_enable = true;
    log_msg.data = std::string("DxlCommunication INFO closeGripper sleep over, stop_gripper_enable");
    g_roslogger_pub.publish(log_msg);
    ...
    while(true) {
        ...
        if(is_gripper_closing){ //如果当前未执行完，则sleep        
            ros::Duration(0.1).sleep();
        }else{
            break;
        }
    }
    // ROS_INFO("DxlCommunication INFO closeGripper now return");
    log_msg.data = std::string("DxlCommunication INFO closeGripper now return");
    g_roslogger_pub.publish(log_msg);
...
int DxlCommunication::scanAndCheck() 
{
    ...
    // 2. Check that ids correspond to gauss motors id list
