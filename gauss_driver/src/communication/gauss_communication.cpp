...
bool GaussCommunication::scanAndCheckMotors()
{
    bool result = true;
    if (can_enabled) {
    ...
    gauss_comm_ok = result;
    return result;
}

bool GaussCommunication::isConnectionOk()
{
    bool result = true;
    if (can_enabled) {
...
void GaussCommunication::manageCanConnectionLoop()
{
    if (!can_enabled) { 
        return; 
    }

    ros::Rate check_connection_rate = ros::Rate(gauss_hw_check_connection_frequency);
        ...
        if (!canComm->isConnectionOk() || new_calibration_requested) {
            ...
            while (!motors_ok) {
                ...
                if ((calibration_step1_result == CAN_STEPPERS_CALIBRATION_OK) 
                    ...
                    // ROS_INFO("calibration failed and try"); 
                     // if calibration is not ok, wait and retry 
                    if ( !(calibration_step1_result == CAN_STEPPERS_CALIBRATION_OK) 
                        && (calibration_step2_result == CAN_STEPPERS_CALIBRATION_OK)) {
                            canComm->setCalibrationFlag(true);
                            ROS_INFO("calibration failed and reset CalibrationFlag to true"); 
                    }
                    // check if connection is still ok
                    if (!canComm->isConnectionOk()) {
                        while (canComm->scanAndCheck() != CAN_SCAN_OK) { // wait for connection to be up
                            ROS_ERROR("Scan to find stepper motors...");
                            ros::Duration(0.25).sleep();
                        }
                    }
                    ...
                    if (calibration_step1_result != CAN_STEPPERS_CALIBRATION_WAITING_USER_INPUT) {
                        ROS_INFO("last calibration has failed, reset flag");
...
void GaussCommunication::checkHardwareVersionFromDxlMotors()
{
...
void GaussCommunication::manageDxlConnectionLoop()
{
    if (!dxl_enabled) {
        return;
    }
...
void GaussCommunication::manageHardwareConnection()
{
    can_connection_loop_thread.reset(new std::thread(boost::bind(&GaussCommunication::manageCanConnectionLoop, this)));
    dxl_connection_loop_thread.reset(new std::thread(boost::bind(&GaussCommunication::manageDxlConnectionLoop, this)));
}

void GaussCommunication::startHardwareControlLoop()
{
    if (can_enabled) { canComm->startHardwareControlLoop(false); }
    if (dxl_enabled) { dxlComm->startHardwareControlLoop(false); }
}

void GaussCommunication::stopHardwareControlLoop()
{
    if (can_enabled) { canComm->stopHardwareControlLoop(); }
    if (dxl_enabled) { dxlComm->stopHardwareControlLoop(); }
}

void GaussCommunication::resumeHardwareControlLoop()
{
    if (can_enabled) { canComm->startHardwareControlLoop(false); }
    if (dxl_enabled) { dxlComm->startHardwareControlLoop(false); }
}

void GaussCommunication::synchronizeMotors(bool begin_traj)
{
    if (can_enabled) {
...
int GaussCommunication::allowMotorsCalibrationToStart(int mode)
{
...
void GaussCommunication::requestNewCalibration()
{
    new_calibration_requested = true;
}

bool GaussCommunication::isCalibrationInProgress()
{
    if (can_enabled) {
...
void GaussCommunication::getHardwareStatus(bool *is_connection_ok, std::string &error_message, 
...
void GaussCommunication::getFirmwareVersions(std::vector<std::string> &motor_names,
...
void GaussCommunication::getCurrentPosition(double pos[6])
{
...
bool GaussCommunication::setLeds(std::vector<int> &leds, std::string &message)
{
    if (leds.size() != 4) {
...
int GaussCommunication::pullAirVacuumPump(uint8_t id, uint16_t pull_air_position, uint16_t pull_air_hold_torque)
{
    if (dxl_enabled) {
...
int GaussCommunication::pushAirVacuumPump(uint8_t id, uint16_t push_air_position)
{
    if (dxl_enabled) {
...
int GaussCommunication::pingAndSetDxlTool(uint8_t id, std::string name)
{
    if (dxl_enabled) {
...
int GaussCommunication::openGripper(uint8_t id, uint16_t open_position, uint16_t open_speed, uint16_t open_hold_torque)
{
    if (dxl_enabled) {
...
int GaussCommunication::closeGripper(uint8_t id, uint16_t close_position, uint16_t close_speed, uint16_t close_hold_torque, uint16_t close_max_torque)
{
    if (dxl_enabled) {
