...
int CanCommunication::init(int hardware_version)
{
    ...
    ros::param::get("/gauss/motors/can_required_motors", required_steppers_ids);
    
    double gear_ratio_1, gear_ratio_2, gear_ratio_3, gear_ratio_4;
    ros::param::get("/gauss/motors/stepper_1_gear_ratio", gear_ratio_1);
    ros::param::get("/gauss/motors/stepper_2_gear_ratio", gear_ratio_2);
    ros::param::get("/gauss/motors/stepper_3_gear_ratio", gear_ratio_3);
    ros::param::get("/gauss/motors/stepper_4_gear_ratio", gear_ratio_4);
    ...
    ros::param::get("/gauss/motors/stepper_1_home_position", home_position_1);
    ros::param::get("/gauss/motors/stepper_2_home_position", home_position_2);
    ros::param::get("/gauss/motors/stepper_3_home_position", home_position_3);
    ros::param::get("/gauss/motors/stepper_4_home_position", home_position_4);
    ...
    ros::param::get("/gauss/motors/stepper_1_offset_position", offset_position_1);
    ros::param::get("/gauss/motors/stepper_2_offset_position", offset_position_2);
    ros::param::get("/gauss/motors/stepper_3_offset_position", offset_position_3);
    ros::param::get("/gauss/motors/stepper_4_offset_position", offset_position_4);
    ...
    ros::param::get("/gauss/motors/stepper_1_direction", direction_1);
    ros::param::get("/gauss/motors/stepper_2_direction", direction_2); 
    ros::param::get("/gauss/motors/stepper_3_direction", direction_3);
    ros::param::get("/gauss/motors/stepper_4_direction", direction_4);

    int max_effort_1, max_effort_2, max_effort_3, max_effort_4;
    ros::param::get("/gauss/motors/stepper_1_max_effort", max_effort_1);
    ros::param::get("/gauss/motors/stepper_2_max_effort", max_effort_2);
    ros::param::get("/gauss/motors/stepper_3_max_effort", max_effort_3);
    ros::param::get("/gauss/motors/stepper_4_max_effort", max_effort_4);
    ...
    for (uint8_t i = 0 ; i < required_steppers_ids.size() ; ++i) {
        if      (required_steppers_ids.at(i) == m1.getId()) { m1.enable(); }
            ...
            debug_error_message = "Incorrect configuration : Wrong ID (" + std::to_string(required_steppers_ids.at(i))
                + ") given in Ros Param /gauss_motors/can_required_motors. You need to fix this !";
            ROS_ERROR("%s", debug_error_message.c_str());
            return -1;
        }
    }
    if (required_steppers_ids.size() == 0) {
        debug_error_message = "Incorrect configuration : Ros Param /gauss_motors/can_required_motors "
    ...
    if (ros::Time::now().toSec() - time_hw_last_write > 1.0/hw_write_frequency) {
        ...
        if (write_torque_on_enable) {
            if (can->sendTorqueOnCommand(CAN_BROADCAST_ID, torque_on) != CAN_OK) {
                ...
                log_msg.data = std::string("gauss_driver ERROR hardwareControlWrite ") + "Failed to send torque on";
        ...
        if (write_synchronize_enable) {
            ...
            if (synchronize_write_success) {
                ...
                std_msgs::String log_msg;
                log_msg.data = std::string("gauss_driver ERROR hardwareControlWrite ") + "Failed to send synchronize position command";
                g_roslogger_pub.publish(log_msg);
            }
        }
        ...
        if (write_micro_steps_enable) {
            ...
            if (micro_steps_write_success) {
                ...
                std_msgs::String log_msg;
                log_msg.data = std::string("gauss_driver ERROR hardwareControlWrite ") + "Failed to send Micro Steps";
                g_roslogger_pub.publish(log_msg);
            }
        }
        ...
        if (write_max_effort_enable) {
            ...
            if (max_effort_write_success) {
                ...
                std_msgs::String log_msg;
                log_msg.data = std::string("gauss_driver ERROR hardwareControlWrite ") + "Failed to send Max Effort";
                g_roslogger_pub.publish(log_msg);
            }
        }
    }
            ...
            if (motors.at(i)->isEnabled()) {
                if (time_now - motors.at(i)->getLastTimeRead() > timeout_read * (motors.at(i)->getHwFailCounter() + 1)) {
                    ...
                    if (motors.at(i)->getHwFailCounter() >= max_fail_counter) {
                        ...
                        std_msgs::String log_msg;
                        log_msg.data = std::string("gauss_driver gauss_driver ERROR ") + debug_error_message;
                        g_roslogger_pub.publish(log_msg);
                        return;
                    }
...
{
    waiting_for_user_trigger_calibration = false;
    ROS_INFO("set waiting_for_user_trigger_calibration to false, no need to calibrate");
...
/* 
 * This flag should be cleared by calling the service /gauss_control/calibrate_motors
 */
...
int CanCommunication::autoCalibrationStep1()
{
    ...
    // 1. Move axis 3 up
    if (relativeMoveMotor(&m3, rad_pos_to_steps(0.2, m3.getGearRatio(), m3.getDirection()), 1000, true) != CAN_OK) {
        return CAN_STEPPERS_CALIBRATION_FAIL; 
    }
...
int CanCommunication::autoCalibrationStep2()
{
    ...
    if (sendCalibrationCommandForOneMotor(&m1, 1000, 1, calibration_timeout) != CAN_OK) {
        return CAN_STEPPERS_CALIBRATION_FAIL;
    }

    if (sendCalibrationCommandForOneMotor(&m2, 200, 1, calibration_timeout) != CAN_OK) {
        return CAN_STEPPERS_CALIBRATION_FAIL;
    }
    ...
    if (relativeMoveMotor(&m1, -m1.getOffsetPosition(), 1000, false) != CAN_OK) {
        return CAN_STEPPERS_CALIBRATION_FAIL;
    }
    //if (relativeMoveMotor(&m2, -m2.getOffsetPosition(), 1000, false) != CAN_OK) {
    //    return CAN_STEPPERS_CALIBRATION_FAIL;
    //}
    if (relativeMoveMotor(&m4, -m4.getOffsetPosition(), 3000, false) != CAN_OK) {
        return CAN_STEPPERS_CALIBRATION_FAIL;
    }
    ...
    if (hardware_version == 1) {
        ros::Duration(abs(m4.getOffsetPosition()) * 5000 / 1000000).sleep();
    }
    ...
    if (hardware_version == 1) {
        if (sendCalibrationCommandForOneMotor(&m3, 500, -1, calibration_timeout) != CAN_OK) {
            return CAN_STEPPERS_CALIBRATION_FAIL;
        }
    ...
    *(calibration_in_progress) = this->calibration_in_progress;
    ...
    for (int i = 0 ; i < motors.size(); i++) {
        if (motors.at(i)->isEnabled()) {
            ...
            motor_types.push_back("Gauss Stepper");
