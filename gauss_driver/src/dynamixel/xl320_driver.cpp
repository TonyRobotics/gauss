...
#include "gauss_driver/dynamixel/xl320_driver.h"
...
//added on 2019-01-07
int XL320Driver::syncWritePgain(std::vector<uint8_t> &id_list, std::vector<uint32_t> &led_list)
{
    return syncWrite1Byte(XL320_ADDR_P_GAIN, id_list, led_list);
}
...
int XL320Driver::syncReadPgain(std::vector<uint8_t> &id_list, std::vector<uint32_t> &hw_error_list)
{
    return syncRead(XL320_ADDR_P_GAIN, DXL_LEN_ONE_BYTE, id_list, hw_error_list);
}
