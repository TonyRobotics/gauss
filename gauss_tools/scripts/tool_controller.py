...
from gauss_msgs.msg import ToolResult
...
class ToolController:
    ...
    def tool_on_goal(self, goal):
        ...
        if cmd.tool_id != self.current_tool.get_id():
            self.server.set_aborted(self.create_action_result(400,
                "Wrong tool selected - ID does not match : " + str(self.current_tool.get_id())))
            return
        ...
        if self.current_tool.is_active:
            ...
            rospy.loginfo("Tool still active, retry later")
            return
	...
	# Check if tool id is null
        ...
        for tool in self.available_tools:
            if new_tool_id == tool.get_id(): 
                if tool.get_type() == "gripper": # extra check for dynamixel tools
                    if not tool.is_connected():
                        return self.create_response(400, "Correct ID but Dynamixel tool is not connected")
                else:
                    self.ros_command_interface.ping_dxl_tool(0, 'No Dxl Tool') # remove dxl tool if another kind of tool is selected
    ...
    def callback_current_tool(self, req):
        return {"id":self.current_tool.id, "type": self.current_tool.get_type()}

    def create_action_result(self, status, message):
        result = ToolResult()
    ...
    def __init__(self, ros_command_interface):
        ...
        self.server = actionlib.SimpleActionServer(
            'gauss/tool_action', ToolAction, self.tool_on_goal, False)
       
        self.change_tool_server = rospy.Service(
            'gauss/change_tool', SetInt, self.callback_change_tool)
        
        self.current_tool_id_server = rospy.Service(
            'gauss/current_tool', CurrentTool, self.callback_current_tool)
      
        self.current_tool_id_publisher = rospy.Publisher(
                'gauss/current_tool_id', Int32, queue_size=1)
